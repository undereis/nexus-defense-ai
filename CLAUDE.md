...
- **API REST para clientes + cliente Delphi:** motor fica em Python; interface visual é CLIENTE sobre a REST. `tools/noc_api.py` (fachada JSON read-only + ações) servida em `api/server.py` sob `/api/*` (gated por token: `/api/overview`, `/subscribers`, `/devices`, `/outages`, `/events`, `/health`; POST `/api/subscribers/{id}/block|unblock`, `/api/billing/run`, `/api/devices/check`). Contrato em `docs/api.md`. Cliente de referência FMX em `clients/delphi/` (`uNexusApi.pas` wrapper REST + `uMainForm.pas` UI-em-código + `NexusClient.dpr`) — escrito, NÃO compilado aqui (sem Delphi no ambiente). Ações de alto risco NÃO entram na REST (só pelo agente/gate).
