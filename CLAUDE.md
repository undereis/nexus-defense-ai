...
- **Agente LangGraph:** `agents/nexus_agent.py` (203 tools registradas)
- **Governança (Control Plane):** `core/` — `control_plane.py` (orquestra: política → auditoria → aprovação → executor; reexporta `ActionRequest/Decision/Result/Risk/Status`) · `policy_engine.py` (decisão determinística ALLOW/DENY/REQUIRE_APPROVAL/DRY_RUN_ONLY + `ACTION_CATALOG`) · `rbac.py` (papéis admin/soc_analyst/noc_operator/auditor/readonly) · `operating_mode.py` (modo do backend real/lab/replay — **fonte da verdade da execução, separada do modo visual do Tauri**) · `redaction.py` (mascara segredo antes de logar) · `secrets.py` (Fase 1: segredos via Keychain do macOS **keychain-first + fallback `.env`**, fail-safe/nunca quebra boot, nunca loga valor; migra com `scripts/nexus_secrets.py`) · `audit_signing.py` (P7: assinatura HMAC LATERAL dos eventos via `AUDIT_HMAC_SECRET`, NÃO toca a hash chain; + export JSON) · `response_playbooks.py` (P9: runbooks DDoS/IP suspeito/honeypot/credential stuffing/queda/drift/Mikrotik/brute force — ações classificadas pela própria policy engine em AUTO/APROVAÇÃO/DRY-RUN/BLOQUEADA) · `models.py`. Inventário autorizado em `tools/asset_registry.py` (tabela `asset_registry`; `check_target` com travas duras p/ loopback/reservado/infra crítica). Casos/incidentes (P6) em `tools/incidents.py` (tabela `incidents`; ciclo open→…→resolved, timeline/evidência/ações, redigido+auditado; **auto-incidente** opt-in `AUTO_INCIDENT_ENABLED` ligado ao hit de honeypot, idempotente+fail-safe). Aprovação **reaproveita** `tools/risk.py` — que agora aplica um **overlay de governança** (`runtime_precheck`: RBAC+segurança+modo) em TODAS as tools de alto risco que funilam por `request_confirmation` (exploit/hydra/sqlmap/mikrotik/bgp/asn/brbos), sem reavaliar toggle. **RBAC na REST** via `config.NEXUS_ROLE_TOKENS` (`require_token` resolve papel; ações passam o papel ao Control Plane; token principal=admin). **Fase 3:** usuários reais em `core/users.py` + tabela `api_users` (token HASHEADO sha256, nunca em claro; papel granular); `require_token` resolve um `Principal` (identidade+papel) na ordem token principal → `NEXUS_ROLE_TOKENS` → usuário do banco; `require_permission(...)` aplica RBAC (403) em `/api/billing/run` e `/api/devices/check`. Operador emite/revoga com `scripts/nexus_users.py`; tool read-only `list_api_users`. Integrado também em `isolate_ip` e `tools/noc_api.block/unblock_subscriber`. **Fase 2:** `run_remote_command` (SSH) e `start/stop_honeypot` roteados por `request_action` (honeypot altera estado → dry-run em lab/replay; SSH read-only roda em lab); `generate_social_engineering_content` por overlay `precheck_runtime` (RBAC+auditoria, sem gate — só gera texto). Doc + TODOs em `docs/control_plane.md`.
...
- **Config central:** `config.py` (todos os toggles de feature; segredos lidos via `core/secrets.get_secret` = Keychain-first + fallback `.env`)
- **Segredos (Keychain, Fase 1):** `core/secrets.py` (Keychain do macOS keychain-first + fallback `.env`, fail-safe, hermético em teste — backend cai p/ `env` sob pytest; nunca loga valor) · `scripts/nexus_secrets.py` (operador: `status`/`migrate`/`set` via getpass sem echo/`delete`) · tool read-only `secret_status_report` · linha no `selftest`. Migrado = Keychain vence o `.env`; nada migrado = comportamento de antes.
- **Usuários da API / RBAC rico (Fase 3):** `core/users.py` (modelo + hash sha256 do token) + tabela `api_users` (só hash+dica) · `scripts/nexus_users.py` (operador: `create`/`list`/`revoke`/`roles` — token exibido só na criação) · tool read-only `list_api_users` · `api/server.Principal`/`require_permission`. Token principal e `NEXUS_ROLE_TOKENS` seguem valendo (compat).
...
- **API REST para clientes + cliente Delphi:** motor fica em Python; interface visual é CLIENTE sobre a REST. `tools/noc_api.py` (fachada JSON read-only + ações) servida em `api/server.py` sob `/api/*` (gated por token: `/api/overview`, `/subscribers`, `/devices`, `/outages`, `/events`, `/health`; POST `/api/subscribers/{id}/block|unblock`, `/api/billing/run`, `/api/devices/check`). Contrato em `docs/api.md`. Cliente desktop de referência (macOS) em `clients/tauri/` — **Tauri v2 + React/TS** (`src/api.ts` wrapper REST via `@tauri-apps/plugin-http`, que dispensa CORS; `src/App.tsx` UI; `src-tauri/` shell Rust) — escrito, NÃO compilado aqui (sem Rust no ambiente). Ações de alto risco NÃO entram na REST (só pelo agente/gate).
